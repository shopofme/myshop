...
//	   echo DB::name('member')->where('mid','111')->value('name');
    ...
    public function table(){
        ...
        print_r($res);
              DB::name('config')->update(['id'=>1,'name'=>'http://localhost']);
        $res = DB::name('config')->cache(true)->find();
        print_r($res);
    ...
    // 同步论坛用户临时测试  http://localhost/home/test/phpwind
    public function phpwind()
    {
         return false;
        $list = Db::connect('mysql://root:@127.0.0.1:3306/myshop#utf8')->query("select * from `tp_users` where oauth = 'qq' and nickname !='' and nickname != 'QQ用户' and user_id > 1");
    ...
    // 发帖  http://localhost/home/test/post_threads
    public function post_threads()
    {
        ...
        */ 
        ...
        // 补丁发布34 -- 版本发布6   6  ==  6 
        // 外包合作52 -- 外包需求23  12 ==  12
        $list = Db::connect('mysql://root:@127.0.0.1:3306/myshop#utf8')->query("select * from tp_article  where cat_id = $cat_id order by article_id desc");
        $list2 = Db::connect('mysql://root:@127.0.0.1:3306/myshop#utf8')->query("select * from `tp_users` where oauth = 'qq' and nickname !='' and nickname != 'QQ用户'  ");
        ...
        foreach($list as $key => $val)
        {
            ...
            $content = str_replace('/Public/upload/article/', 'http://localhost/Public/upload/article/', $content);
    ...
    // 帖子回复 http://localhost/home/test/comment
    public function comment(){
        ...
        $list = Db::connect('mysql://root:@127.0.0.1:3306/myshop#utf8')->query("select * from myshop.`tp_comment` where goods_id in(select article_id from tp_article  where cat_id = $cat_id) ");
        $list2 = Db::connect('mysql://root:@127.0.0.1:3306/myshop#utf8')->query("select * from `tp_users` where oauth = 'qq' and nickname !='' and nickname != 'QQ用户'");
...
// 更改帖子评论数和浏览数 http://localhost/home/test/updatecomment
