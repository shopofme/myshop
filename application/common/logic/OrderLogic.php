...
class OrderLogic
{
	...
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='')
	{
		...
		$shipping = M('Plugin')->where("code", $shipping_code)->cache(true,MYSHOP_CACHE_TIME)->find();
		...
		{
			$goods = M('goods')->where("goods_id", $val['goods_id'])->cache(true,MYSHOP_CACHE_TIME)->find();
